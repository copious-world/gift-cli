 ...
 */
function load_config_file(conf_file : string) : false | Object {
    console.log("CONFIG FILE: ",conf_file)
    let config : false | Object = false
    try {
        let confbuf : Buffer = readFileSync(conf_file);
        let confstr : string  = confbuf.toString()
